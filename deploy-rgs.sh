...
if [[ -z $DEPLOY_CONFIG ]]; then
	printf "Missing environment variables: DEPLOY_CONFIG \n"
	exit 1;
fi
...
service_name="gengame/$service_id" # name of krug service without prefix "krug-". e.g. krug-gateway => gateway
...
docker_cmd="$docker_cmd $service_image"

# pull new image and run it
echo "pull and run image $service_image: $docker_cmd"
eval $docker_cmd

# docker run -d --name nurgs --net host -e SPRING_PROFILES_ACTIVE=gpkint-rgs -e HEAP_SIZE=4G -e API_ACCESS=ANYTHINGGOOD -e JMX_BIND_INTERFACE=eth0 -e USE_G1GC=1 -v "/data/logs:/data/logs" -e lvl_prog=1 gengame/nurgs:1.7.7-loadtest-nocache
# docker run -d --name rgs   --net host -e PROFILE=gpkint                    -e HEAP_SIZE=4G -e API_ACCESS=ANYTHINGGOOD -e JMX_BIND_INTERFACE=eth0 -e USE_G1GC=1 -v /var/log/m4:/var/log/m4 -p 80:8081    gengame/rgs:7.21.0-rc1
